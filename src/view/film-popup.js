...
const addComments = (comments) => comments.map(({author, date, text, emotion}) =>
...
const createFilmPopupElement = (data) => {
  const {title, originalTitle, poster, description, rating, director, writer, country, ageRating, actor, time, date, genre, isWatchlist, isWatched, isFavorite, comments, isEmoji} = data;
...
export default class FilmPopupView extends SmartView {
  ...
  get template() {
    return createFilmPopupElement(this._data);
  }
  ...
  reset = () => {
    this.updateData(
      FilmPopupView.parseFilmToData(this.#film)
    );
  }
}
